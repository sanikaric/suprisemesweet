<?php
class beoreo_PostLove {
	public $post_id;
	function __construct($post_id = null)   {
		$this->post_id = $post_id;
        add_action('wp_enqueue_scripts', array($this, 'enqueue_scripts'));
        add_action('wp_ajax_nectar-love', array($this, 'beoreo_ajax_love'));
		add_action('wp_ajax_nopriv_nectar-love', array($this, 'beoreo_ajax_love'));
	}
	...
	function beoreo_ajax_love($post_id) {
		//update
	...
	function beoreo_love_post($post_id, $action = 'get')
	{
	...
	function add_love() {
		global $post;
		$output = $this->beoreo_love_post($post->ID);
