<?php
class beoreo_Social_Widget extends WP_Widget {
    function __construct() {
        parent::__construct(
                'social_widget', // Base ID
                __('Socials', 'beoreo'), // Name
                array('description' => __('Social Widget', 'beoreo'),) // Args
        );
    }
    ...
    function form($instance) {
		...
		?>
		<p>
            <label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php _e('Title:', 'beoreo'); ?></label>
        ...
        for ($i = 1; $i <= 12; $i++) {
            ?>
            <p>
                <label for="<?php echo esc_url($this->get_field_id('icon_social_' . $i)); ?>"><?php _e('Social Icon:', 'beoreo');
            echo $i; ?></label>
            ...
            <p>
                <label for="<?php echo esc_url($this->get_field_id('link_social_' . $i)); ?>"><?php _e('Social Link:', 'beoreo');
            echo $i; ?></label>
        ...
        <?php } ?>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('el_class')); ?>"><?php _e('Extra Class:', 'beoreo'); ?></label>
 ...
 */
function beoreo_register_social_widget() {
    register_widget('beoreo_Social_Widget');
}
add_action('widgets_init', 'beoreo_register_social_widget');
?>
